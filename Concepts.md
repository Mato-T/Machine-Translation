...
## Decoder
- As done with the encoder inputs, positional encoding is added to those decoder inputs to indicate the position of each word. The input to the decoder is the same as for the encoder: the input embedding along with positional encoding. However, there is a difference in how attention is calculated
