...
## Decoder
- The input to the decoder is the same as for the encoder: the input embedding along with positional encoding. However, there is a difference in how attention is calculated. In the decoder, the self-attention layer is only allowed to calculate the context from earlier positions in the translated sequence. This is done by masking future positions before the softmax step in the self-attention calculations
